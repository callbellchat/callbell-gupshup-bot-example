...
function ScriptHandler(context, event) {
  ...
  console.log("CONTEXT", context.simpledb.roomleveldata.desactivateBot);
  if (event.message.toLowerCase() == "hablar con el bot") {
    context.simpledb.roomleveldata.desactivateBot = false;
  }

  var desactivateBot = context.simpledb.roomleveldata.desactivateBot;

  options.success = function (opm) {
    if (!desactivateBot) {
      context.sendResponse(JSON.stringify(opm));
    } else {
      callbell.sendToCallbell(
        options,
        context,
        event,
        event.message,
